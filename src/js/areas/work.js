...

{
	...
	dispatch(event) {
		...
		switch (event.type) {
			...
			case "switch-view":
				if (Self.activeView) {
					// notify current view of exit event
					APP[Self.activeView].dispatch({ type: "switch-exit-event" });
				}
				// update buttons
				event.el.parent().find(".active").removeClass("active");
				event.el.addClass("active");
				// notify area of switch
				APP[event.el.data("arg")].dispatch({ type: "switch-enter-event" });
				Self.activeView = event.el.data("arg");
				// updat work area
				APP.els.content
